#!/usr/bin/env python3
"""Record Milestone 9 implementation/evidence completion without closing release CI."""
...
old_status = "**Status:** Ralph Loop active — `v0.2.0` released; Milestone 8 complete; Milestone 9 active"
new_status = "**Status:** Ralph Loop active — `v0.2.0` released; Milestone 9 implementation/evidence complete; v0.3 exact release gate pending"
if text.count(old_status) != 1:
    raise SystemExit(f"expected one top status, found {text.count(old_status)}")
text = text.replace(old_status, new_status, 1)

old = """## Milestone 9 — Targeted metrics
...
"""
new = """## Milestone 9 — Targeted metrics
...
"""
if text.count(old) != 1:
    raise SystemExit(f"expected one Milestone 9 block, found {text.count(old)}")
