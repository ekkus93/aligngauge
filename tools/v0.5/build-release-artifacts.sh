...
for executable in cargo find git gzip install python3 sha256sum tar touch; do
...
CARGO_TARGET_DIR="$staging/cargo-target" \
...
install -m 0755 \
  "$staging/cargo-target/release/aligngauge" \
