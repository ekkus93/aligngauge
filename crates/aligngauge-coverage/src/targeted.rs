...
impl SourceTargetState {
    ...
    fn observe_segment(
        ...
        if start != self.next_position {
            return Err(internal_error(
                "per-target coverage segments are not contiguous",
            )
        ...
        for (index, threshold) in thresholds.iter().enumerate() {
            if depth >= u64::from(*threshold) {
                let value = self.threshold_bases.get_mut(index).ok_or_else(|| {
                    internal_error("per-target threshold state is missing")
                })?;
                *value = value
                    .checked_add(bases)
    ...
    fn close_zero_run(
        ...
        *total_zero_runs = total_zero_runs
        ...
        self.longest_zero_coverage_run_bases =
            self.longest_zero_coverage_run_bases.max(length);
        self.zero_runs
            .push(ZeroCoverageRunSummary { start, end });
        Ok(())
    }
...
impl ReferenceTargetState {
    fn finalize_layout(&mut self) {
        self.target_union.sort_by_key(|interval| (interval.start, interval.end));
        self.selected_union
            .sort_by_key(|interval| (interval.start, interval.end));
        self.sources.sort_by_key(|source| {
            (source.start, source.end, source.source_index)
        });
    }
    ...
    fn observe_source_run(
        ...
        for index in active_sources.iter().copied() {
            let source = sources.get_mut(index).ok_or_else(|| {
                internal_error("active source-target index is invalid")
            })?;
...
impl TargetedReducer {
    pub(crate) fn new(
        ...
        let genome_territory_bases = header.references().iter().try_fold(
            0_u64,
            |total, reference| {
                total
    ...
    pub(crate) fn observe_run(
        ...
        let target_overlap = overlap_union(
            &state.target_union,
    ...
    pub(crate) fn finish(
        ...
        for state in self.references.into_values() {
...
pub(crate) fn memory_reservation(
    ...
    let max_zero_runs = usize::try_from(
        TARGET_ZERO_RUN_BUDGET_BYTES / ZERO_RUN_ESTIMATED_BYTES,
    )
    .map_err(|source| {
...
fn target_depth_20th_percentile(
    ...
    let rank = territory / 5 + u64::from(territory % 5 != 0);
