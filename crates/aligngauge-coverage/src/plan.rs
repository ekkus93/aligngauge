...
impl CoverageMemoryPlan {
    ...
    pub fn plan(
        ...
        let minimum_delta_bytes = delta_bytes(MIN_CHUNK_BASES, bytes_per_chunk_base)?;
        let minimum_planned = fixed_bytes
        ...
        let available_for_delta = memory_limit_bytes - fixed_bytes;
        let maximum_by_memory = available_for_delta
            .checked_div(bytes_per_chunk_base)
            .and_then(|entries| entries.checked_sub(1))
            .ok_or_else(|| resource_error("memory limit cannot hold a coverage delta vector"))?;
        let maximum_by_memory = usize::try_from(maximum_by_memory).map_err(|source| {
        ...
        let delta_bytes = delta_bytes(chunk_size_bases, bytes_per_chunk_base)?;
        let planned_peak_bytes = fixed_bytes
        ...
        let max_pending_event_positions = usize::try_from(
            pending_event_budget_bytes / PENDING_EVENT_ENTRY_BYTES,
        )
        .map_err(|source| {
            resource_error("pending-event capacity does not fit usize").with_source(source)
        })?;
        let max_histogram_bins = usize::try_from(histogram_budget_bytes / HISTOGRAM_ENTRY_BYTES)
            .map_err(|source| {
                resource_error("histogram capacity does not fit usize").with_source(source)
            })?;
    ...
    pub fn to_json(&self) -> JsonValue {
        JsonValue::Object(BTreeMap::from([
            (
                ...
                JsonValue::Unsigned(
                    u64_from_usize(self.active_tracks, "active tracks")
                        .expect("validated active track count fits u64"),
                ),
            ),
            (
                String::from("chunk_size_bases"),
                JsonValue::Unsigned(
                    u64_from_usize(self.chunk_size_bases, "chunk size")
            ...
            (
                ...
                JsonValue::Unsigned(
                    u64_from_usize(self.max_histogram_bins, "histogram bins")
                        .expect("validated histogram capacity fits u64"),
                ),
            ),
            (
                String::from("max_pending_event_positions"),
                JsonValue::Unsigned(
                    u64_from_usize(self.max_pending_event_positions, "pending event positions")
                        .expect("validated pending-event capacity fits u64"),
                ),
            ),
            (
